(() => {
  ...
  const layerGroups = {biomes:["layer-biomes"],climate:["layer-climate-transitions"],territories:["layer-territories"],regions:["layer-regions","layer-region-labels"],routes:["layer-routes"],hydrology:["layer-lakes","layer-rivers"],relief:["layer-relief"],settlements:["layer-settlements","layer-natural-ports"],landmarks:["layer-landmarks"],unknown:["layer-unknown-regions"]};
  ...
  const writeState = () => {
    const params=new URLSearchParams(),active=layerInputs.filter(input=>input.checked).map(input=>input.dataset.layer);
    if(selectedFeatureId)params.set("feature",selectedFeatureId);params.set("layers",active.join(","));history.replaceState(null,"",`#${params}`);
  };
  const rebuildSuggestions = () => {
    const selectedCategory=category.value,available=[...features.values()].filter(feature=>selectedCategory==="all"||feature.properties.collection===selectedCategory);
    suggestions.replaceChildren(...available.sort((a,b)=>a.properties.name.localeCompare(b.properties.name,"pt-BR")).map(feature=>{const option=document.createElement("option");option.value=feature.properties.name;option.dataset.id=feature.id;return option;}));
    search.value="";setStatus(`${available.length} elementos disponíveis no filtro.`);
  };
  ...
  const showFeature = id => {
    ...
    const atlasEntry=atlasByEntity.get(id),atlasLink=document.createElement("a"),sceneEntry=sceneByEntity.get(id),sceneLink=document.createElement("a"),sceneState=document.createElement("p");
    ...
    if(sceneEntry){sceneLink.href=`${sceneEntry.url}#entity=${encodeURIComponent(id)}`;sceneLink.textContent="Explorar em 3D";}
    else if(p.collection==="settlements"){sceneState.className="detail-empty";sceneState.textContent="Cena 3D ainda não disponível para este assentamento.";}
    detail.replaceChildren(title,list,...(atlasEntry?[atlasLink]:[]),...(sceneEntry?[sceneLink]:p.collection==="settlements"?[sceneState]:[]));
  ...
  async function initialize(){
    ...
    const [geojson,atlas,sceneRegistry]=await Promise.all([geoResponse.json(),atlasResponse.json(),sceneResponse.json()]);features=new Map(geojson.features.map(feature=>[feature.id,feature]));
    ...
    sceneByEntity=new Map(sceneRegistry.scenes.map(scene=>[scene.entityId,scene]));
    rebuildSuggestions();
    ...
    const state=new URLSearchParams(location.hash.slice(1)),savedLayers=state.get("layers");if(savedLayers!==null){const active=new Set(savedLayers.split(","));layerInputs.forEach(input=>{input.checked=active.has(input.dataset.layer);});}
    layerInputs.forEach(input=>applyLayer(input));const selected=state.get("feature");if(selected)showFeature(selected);setStatus("Mapa interativo carregado. Use os controles, o mouse ou o teclado.");
  }
  ...
  layerInputs.forEach(input=>input.addEventListener("change",()=>applyLayer(input,true)));category.addEventListener("change",rebuildSuggestions);
  document.querySelector("#clear-selection").addEventListener("click",()=>{if(selectedTarget)selectedTarget.style.filter="";selectedTarget=undefined;selectedFeatureId=undefined;detail.replaceChildren(Object.assign(document.createElement("h2"),{textContent:"Detalhes"}),Object.assign(document.createElement("p"),{textContent:"Selecione um elemento no mapa ou use a pesquisa."}));writeState();setStatus("Seleção limpa.");});
